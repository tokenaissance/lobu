...
Lobu boots as a single Node process. Bring your own Postgres + Redis (managed instances or local — `brew services start postgresql redis` works for dev).
...
# 2. Set DATABASE_URL and REDIS_URL in .env, then boot
...
├── .env                       # secrets (API keys, DATABASE_URL, REDIS_URL)
