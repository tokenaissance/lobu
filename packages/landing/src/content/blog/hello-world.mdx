...
**Skills** are modular bundles of instructions, MCP servers, system packages, and network requirements. A skill declares what it needs: integrations, Nix packages, and domains to allowlist. Tool visibility and approval policy live separately in `lobu.toml`, which keeps the capability manifest distinct from security controls. Lobu and Owletto now ship separate public skills you can install with `npx skills add lobu-ai/lobu --skill lobu --agent openclaw -y` and `npx skills add lobu-ai/lobu --skill owletto --agent openclaw -y`. Teams can still create project-owned local skills, and agents can request skill installation mid-conversation — the user gets a prefilled settings link, approves, and the agent resumes.
