...
export class SlackEventHandlers {
  ...
  private setupDirectMessageHandler(): void {
    ...
    this.app.message(async ({ message, client }) => {
      ...
      logger.info(
        `Message event received - channel: ${event.channel}, channel_type: ${event.channel_type}, subtype: ${event.subtype}, user: ${event.user}, thread_ts: ${event.thread_ts}, files: ${event.files?.length || 0}, eventKeys: ${Object.keys(event).join(",")}`
      );
