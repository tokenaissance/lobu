...
const DEFAULT_SLACK_MANIFEST = {
  ...
  features: {
    ...
    slash_commands: [
      {
        command: "/termos",
        description: "Termos commands - manage repositories and authentication",
...
export async function initCommand(
  ...
  const baseAnswers = await inquirer.prompt([
    {
      ...
      default: projectNameArg || "my-termos",
  ...
  const { slackAppOption } = await inquirer.prompt([
    {
      ...
      choices: [
        {
          name: "Create a new Slack app using the Termos manifest",
          value: "create",
        },
        {
  ...
  const spinner = ora("Creating Termos project...").start();

  try {
    const workerMode = answers.workerMode;

    // Create .termos directory in project directory
    const termosDir = join(projectDir, ".termos");
    await mkdir(termosDir, { recursive: true });
    ...
    if (answers.selectedMcpServers.length > 0) {
      ...
      await writeFile(
        join(termosDir, "mcp.config.json"),
    ...
    if (answers.selectedMcpServers.length > 0) {
      console.log(chalk.dim("     - .termos/mcp.config.json"));
    }
...
function generateDockerCompose(options: {
  ...
  const gatewayImage = `buremba/termos-gateway:latest`;

  const mcpConfigMount = hasMcpServers
    ? `
      - ./.termos/mcp.config.json:/app/.termos/mcp.config.json:ro`
    : "";

  const mcpEnvVars = hasMcpServers
    ? `
      MCP_CONFIG_URL: file:///app/.termos/mcp.config.json
      ENCRYPTION_KEY: \${ENCRYPTION_KEY}`
    : "";

  return `# Generated by @termosdev/cli
...
services:
  ...
  gateway:
    ...
    volumes:
      ...
      - env_storage:/app/.termos/env
    networks:
      - termos-public   # Internet access
      - termos-internal # Internal services (redis, workers)
    depends_on:
      redis:
